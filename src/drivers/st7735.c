...
{
  //Output: CS_LD
  ST7735_DDR_CS_LD |= (1 << ST7735_CS_LD);  
  //Output: DC_LD
  ST7735_DDR_DC_LD |= (1 << ST7735_DC_LD); 
...
void St7735SetBAckligth(void)
{	
	// set DDR BackLigt
	ST7735_DDR_BL |= (1 << ST7735_BL);
	// set high level on Backlight
	ST7735_PORT_BL |= (1 << ST7735_BL);
}
...
uint8_t CommandSend(uint8_t data)
{
  ...
  ST7735_PORT_CS_LD &= ~(1 << ST7735_CS_LD);
  // command (active low)
  ST7735_PORT_DC_LD &= ~(1 << ST7735_DC_LD);
  // transmitting data
  SPDR = data;
